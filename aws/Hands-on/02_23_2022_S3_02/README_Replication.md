...
- Create a new bucket named `pet.poyraz.lifecycle` with following properties.
...
- Click the S3 bucket `pet.poyraz.lifecycle` and upload following files.
...
- Create a bucket of `source.replica.poyraz` with following properties,
...
- Click the S3 bucket `source.replica.poyraz` and upload following files.
...
            "Principal": "*",
            "Action": "s3:GetObject",
            "Resource": "arn:aws:s3:::pet-store-bucket-poyraz/*"
        }
    ]
}
```
...
- Create a new bucket named `destination.cross.region.replica.poyraz` with following properties.
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Resource": [
                "arn:aws:s3:::source.replica.poyraz",
                "arn:aws:s3:::source.replica.poyraz/*"
            ]
        },
        {
            ...
            "Effect": "Allow",
            "Resource": "arn:aws:s3:::destination.cross.region.replica.poyraz/*"
        }
    ]
}
```
...
```text
Name            : poyraz.cross.replication.iam.policy

Description     : poyraz.cross.replication.iam.policy
```
...
- Enter `poyraz.cross.replication.iam.policy` in filter policies box and select the policy
...
```text
Role Name           : poyraz.cross.replication.iam.role
Role Description    : poyraz.cross.replication.iam.role
```
...
4. Destination                      : 
   ...
   - Bucket name : destination.cross.region.replica.poyraz
5. IAM role                         : poyraz.cross.replication.iam.role
...
- Go to `source.replica.poyraz` bucket and upload `index.html` and `cat.jpg` again.

- Go to `destination.cross.region.replica.poyraz` bucket, copy `Endpoint` and paste to browser.
...
d. Destination                      : 
   - Choose a bucket in this account
   - Bucket name : destination.cross.region.replica.poyraz
e. IAM role                         : poyraz.cross.replicationiam.   role
...
- Go to `source.replica.poyraz` bucket and upload `outside_folder.txt`.

- Go to `destination.cross.region.replica.poyraz` bucket and show that the`outside_folder.txt` is not replicated in the bucket.

- This time go to `source.replica.poyraz` bucket >> `kitten` folder and upload `inside_kitten_folder.txt`.

- Go to `destination.cross.region.replica.poyraz` bucket show that `kitten` folder has been replicated automatically due to replication rule and inside the folder there is `inside_kitten_folder.txt file`.
...
- Go to `source.replica.poyraz` bucket and direct upload `tag_file.txt`.

- Go to `destination.cross.region.replica.poyraz` bucket and show that the `tag_file.txt` is not replicated in the bucket.

- This time go to `source.replica.poyraz` bucket, and upload the `tag_file.txt` file with tag as `key:name` and `value:cat`;
...
- Go to `destination.cross.region.replica.poyraz` bucket show that the `tag_file.txt` file is replicated.
